    ...
    if not df_mouvs.empty:
        ...
        for _, row in df_mouvs.iterrows():
            ...
            if not df_clients.empty:
                ...
                if not df_tel.empty:
                    tel = str(df_tel.iloc[0].get('Numéro de téléphone', 'N/A'))
...
with tab_vidange:
    ...
    if not df_vidanges.empty:
        ...
        df_tableau_affichage = df_v_base[['Date_Mise_A_Jour', 'Marque', 'Matricule', 'Date_Dernier_Vidange', 'KM_Dernier_Vidange', 'KM_Recent', 'KM cerculer', 'km restant']].rename(
            columns={
                'Date_Mise_A_Jour': 'DATE MIS AJOURS',
                'Marque': 'MARQUE',
                'Matricule': 'MATRICULE',
                'Date_Dernier_Vidange': 'TE DERNIER VIDAN',
                'KM_Dernier_Vidange': 'DERNIER VIDA',
                'KM_Recent': 'KM RECENT',
                'KM cerculer': 'KM cerculer',
                'km restant': 'km restant'
            }
        )
                        ...
                        if st.session_state.get(f"mode_edition_{unique_suffix}", False):
                            ...
                            with st.form(key=f"form_reel_edit_{unique_suffix}"):
                                ...
                                if st.form_submit_button("✅ METTRE À JOUR"):
                                    ...
                                    if f_cin_remplace:
                                        client_update["Image CIN"] = encoder_image_base64(f_cin_remplace)
                                    if f_per_remplace:
