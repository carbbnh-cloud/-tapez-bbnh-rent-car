...
/* 🆕 SIDEBAR COMPACTE : largeur réduite sur mobile */
section[data-testid="stSidebar"] {
    background-color: #07080a !important;
    border-right: 1px solid #1f242e !important;
}

/* 🆕 TABS COMPACTS : plus petits et plus serrés */
.stTabs [data-baseweb="tab-list"] {
    gap: 4px; background-color: #161920; padding: 4px;
    border-radius: 10px; border: 1px solid #222733; margin-bottom: 15px;
...
.stTabs [data-baseweb="tab"] {
    padding: 6px 10px !important; border-radius: 8px; font-weight: 600;
...
h1 { font-weight: 800 !important; letter-spacing: -1px !important; color: #ffffff !important; font-size: 20px !important; margin-bottom: 10px !important; }
h3 { color: #f3f4f6; font-weight: 700 !important; letter-spacing: -0.5px; font-size: 15px !important; margin-bottom: 8px !important; }

div[data-testid="stForm"] {
    background: rgba(22, 25, 32, 0.8) !important;
    border: 1px solid #2a3142 !important; padding: 12px !important;
...
div.stButton > button:hover {
    ...
    box-shadow: 0 3px 10px rgba(230, 0, 0, 0.5);
}
...
/* STYLES ALERTES COMPACTS */
.alerte-urgent {
    background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%);
    color: white; padding: 10px; border-radius: 8px;
    margin: 8px 0; font-weight: bold; border-left: 4px solid #fbbf24; font-size: 12px;
}
.alerte-warning {
    background: linear-gradient(135deg, #f97316 0%, #c2410c 100%);
    color: white; padding: 8px; border-radius: 8px;
    margin: 6px 0; font-weight: 600; font-size: 12px;
}
.alerte-ok {
    background: linear-gradient(135deg, #16a34a 0%, #166534 100%);
    color: white; padding: 6px; border-radius: 8px; margin: 4px 0; font-size: 12px;
}
.jours-restants-urgent, .jours-restants-warning, .jours-restants-ok {
    color: white !important; font-weight: bold !important;
    padding: 3px 8px; border-radius: 4px; font-size: 11px; display: inline-block;
}
...
/* 🆕 MEDIA QUERIES MOBILE : TOUT COMPACT */
@media (max-width: 768px) {
    input, select, textarea { font-size: 16px !important; }
    
    /* Sidebar en tiroir compact (80% de largeur max) */
    section[data-testid="stSidebar"] { 
        width: 85% !important; 
    ...
    div[data-testid="stExpander"] summary { font-size: 13px !important; padding: 6px !important; }
}
</style>
...
def generer_alerte(jours_restants):
    ...
    if jours_restants < 0: return f'<span class="jours-restants-urgent">🔴 ÉCHU {-jours_restants}j</span>'
    elif jours_restants == 0: return f'<span class="jours-restants-urgent">🔴 AUJOURD\'HUI</span>'
    elif jours_restants <= 15: return f'<span class="jours-restants-urgent">⚠️ {jours_restants}j</span>'
    elif jours_restants <= 30: return f'<span class="jours-restants-warning">🟠 {jours_restants}j</span>'
    else: return f'<span class="jours-restants-ok">✅ {jours_restants}j</span>'
...
with st.sidebar:
    ...
    if os.path.exists(logo_path):
        ...
        st.markdown("<p style='text-align: center; opacity: 0.5; font-size:11px; letter-spacing:3px; margin-bottom:15px;'>RENT A CAR</p>", unsafe_allow_html=True)

    if st.button("🔄 RAFRAÎCHIR", use_container_width=True):
        st.session_state["force_reload"] = True
        rerun()
    
    st.markdown("<h3 style='margin-bottom:8px;'>🕹️ ACTIONS</h3>", unsafe_allow_html=True)
...
# 📝 NOUVEAU CONTRAT
# ============================================================
if menu_action == "📝 Nouveau":
    st.sidebar.markdown("### 📝 Nouvelle fiche")
    nature = st.sidebar.selectbox("Nature", ["Contrat", "Réservation", "Maintenance"])
    vehicule = st.sidebar.selectbox("Véhicule", liste_vehicules_opt) if liste_vehicules_opt else st.sidebar.text_input("Matricule")
    client_b = st.sidebar.selectbox("Client", liste_clients_opt)
    nom_m = st.sidebar.text_input("Nom & Prénom")
    cin_m = st.sidebar.text_input("N° CIN")
    dc_m = st.sidebar.date_input("Date CIN", datetime.now() - timedelta(days=365))
    permis_m = st.sidebar.text_input("N° Permis")
    dp_m = st.sidebar.date_input("Date Permis", datetime.now() - timedelta(days=365))
    f_cin = st.sidebar.file_uploader("Image CIN", type=["png", "jpg", "jpeg"])
    f_permis = st.sidebar.file_uploader("Image Permis", type=["png", "jpg", "jpeg"])
    st.sidebar.markdown("---")
    ...
    st.sidebar.markdown(f"**🔢 Durée :** `{nbr_jours}j`")
    prix_unitaire = st.sidebar.number_input("💰 Prix/Jour (DT)", min_value=0, value=100, step=5)
    montant_total = st.sidebar.number_input("💵 Total (DT)", min_value=0, value=int(nbr_jours * prix_unitaire))
    caution = st.sidebar.number_input("🛡️ Caution (DT)", value=0)
    reste = montant_total - caution
    st.sidebar.markdown(f"**🔴 Reste :** `{reste} DT`")
    st.sidebar.markdown("---")
    km_debut = st.sidebar.number_input("KM Départ", min_value=0, value=0, step=1)
    l_reception = st.sidebar.text_input("Lieu réception", value="Siège Monastir")
    no_vol = st.sidebar.text_input("N° vol", value="")
    info_note = st.sidebar.text_area("Note")
    ref = st.sidebar.text_input("Code Contrat", f"BBNH-{datetime.now().strftime('%d%H%M%S')}")

    if st.sidebar.button("⚡ ENREGISTRER", use_container_width=True):
        try:
            if client_b == "-- Manuel --": nom_f, cin_f = nom_m, cin_m
            else:
                nom_f, cin_f = parse_client_label(client_b)
                if nom_m: nom_f = nom_m
                if cin_m: cin_f = cin_m
            if not vehicule or not vehicule.strip(): st.error("❌ Véhicule obligatoire")
            elif not nom_f or not nom_f.strip(): st.error("❌ Nom obligatoire")
            else:
...
elif menu_action == "🚗 + Véhicule":
    with st.sidebar.form("form_add_car"):
        st.markdown("### 🚗 Nouveau")
        nouveau_matricule = st.text_input("Matricule *").strip()
...
elif menu_action == "🗑️ - Véhicule":
    if liste_vehicules_complets_opt:
        with st.sidebar.form("form_del_car"):
            vehicule_a_retirer = st.selectbox("Supprimer", liste_vehicules_complets_opt)
            confirmer_suppression = st.checkbox("Confirmer")
            if st.form_submit_button("💥 SUPPRIMER", use_container_width=True):
                if confirmer_suppression:
                    ...
                    get_all_tables.clear()
                    rerun()
    else: st.sidebar.info("Aucun véhicule")

elif menu_action == "⚙️ Modifier":
    ...
    if not df_mouv_all.empty and MOUV_KEY_COLUMN in df_mouv_all.columns:
        ...
        if liste_mouv_mod:
            ...
            mouv_selectionne = st.sidebar.selectbox("Dossier", liste_mouv_mod, key="mod_selectbox", index=liste_mouv_mod.index(st.session_state["mod_selected_label"]))
            st.session_state["mod_selected_label"] = mouv_selectionne
            selected_key = key_map.get(mouv_selectionne, '')
            if not selected_key: st.sidebar.error("❌ Invalide")
            else:
                row_matches = df_mouv_all[df_mouv_all[MOUV_KEY_COLUMN].astype(str).str.strip() == selected_key.strip()]
                if row_matches.empty: st.sidebar.error(f"❌ Introuvable")
                else:
                    ...
                    nature_options = ["Location", "Réservation", "Maintenance"]
                    ...
                    st.sidebar.markdown(f"### ⚙️ {selected_key}")
                    ...
                    st.sidebar.markdown("👤 **Conducteur**")
                    mod_client = st.sidebar.text_input("Nom", value=init_client, key=f"mod_cli_{selected_key}")
                    mod_cin = st.sidebar.text_input("CIN", value=init_cin, key=f"mod_cin_{selected_key}")
                    mod_date_cin = st.sidebar.date_input("Date CIN", init_date_cin, key=f"mod_dc_{selected_key}")
                    mod_permis = st.sidebar.text_input("Permis", value=init_permis, key=f"mod_perm_{selected_key}")
                    mod_date_permis = st.sidebar.date_input("Date Permis", init_date_permis, key=f"mod_dp_{selected_key}")
                    st.sidebar.markdown("---")
                    c_d1, c_t1 = st.sidebar.columns(2)
                    with c_d1: mod_d1 = st.sidebar.date_input("Début", init_date_deb, key=f"mod_d1_{selected_key}")
                    with c_t1: mod_t1 = st.sidebar.time_input("Heure", init_time_deb, key=f"mod_t1_{selected_key}")
                    c_d2, c_t2 = st.sidebar.columns(2)
                    with c_d2: mod_d2 = st.sidebar.date_input("Fin", init_date_fin, key=f"mod_d2_{selected_key}")
                    with c_t2: mod_t2 = st.sidebar.time_input("Heure", init_time_fin, key=f"mod_t2_{selected_key}")
                    mod_nbr_jours = max(1, (mod_d2 - mod_d1).days)
                    st.sidebar.markdown(f"**🔢 Durée :** `{mod_nbr_jours}j`")
                    ...
                    mod_prix_unitaire = st.sidebar.number_input("💰 Prix/Jour", min_value=0, value=init_prix_unit_calc if init_prix_unit_calc > 0 else 100, key=f"mod_pu_{selected_key}")
                    mod_prix = st.sidebar.number_input("Total", value=int(mod_nbr_jours * mod_prix_unitaire), key=f"mod_tot_{selected_key}")
                    mod_caution = st.sidebar.number_input("Caution", value=int(init_caution), key=f"mod_cau_{selected_key}")
                    mod_reste = mod_prix - mod_caution
                    st.sidebar.markdown(f"**🔴 Reste :** `{mod_reste} DT`")
                    st.sidebar.markdown("---")
                    mod_lieu = st.sidebar.text_input("Lieu", value=init_lieu, key=f"mod_lieu_{selected_key}")
                    mod_vol = st.sidebar.text_input("Vol", value=init_vol, key=f"mod_vol_{selected_key}")
                    mod_km_deb = st.sidebar.number_input("KM", min_value=0, value=init_km_deb, key=f"mod_km_{selected_key}")
                    mod_note = st.sidebar.text_area("Note", value=init_note, key=f"mod_note_{selected_key}")
                    if st.sidebar.button("💾 SAUVER", key=f"mod_save_{selected_key}", use_container_width=True):
                        try:
                            ...
                            if ok_update:
                                ...
                                st.sidebar.success("✅ Sauvé !")
                                get_all_tables.clear()
                                st.session_state["force_reload"] = True
                                rerun()
                            else: st.sidebar.error("❌ Échec")
                        except Exception as e:
                            st.sidebar.error(f"❌ Erreur : {e}")
                            traceback.print_exc()
        else: st.sidebar.info("📭 Aucun dossier")
    else: st.sidebar.info("📭 Aucun dossier")

elif menu_action == "❌ Supprimer":
    ...
    if not df_mouv_actifs.empty and MOUV_KEY_COLUMN in df_mouv_actifs.columns:
        ...
        if liste_mouv_del:
            with st.sidebar.form("form_del_mouv"):
                mouv_selectionne = st.selectbox("Opération", liste_mouv_del)
                confirmer_action = st.checkbox("Confirmer")
                if st.form_submit_button("💥 SUPPRIMER", use_container_width=True):
                    if confirmer_action:
        ...
        else: st.sidebar.info("📭 Aucune opération")
    else: st.sidebar.info("Aucune opération")

elif menu_action == "🔄 Résa→Contrat":
    st.sidebar.markdown("### 🔄 Résa → Contrat")
    st.sidebar.info("💡 Convertit une **Réservation** en **Contrat**")
    df_mouvs_fresh = fetch_table_direct(T_MOUVEMENT)
    if not df_mouvs_fresh.empty:
        st.sidebar.markdown(f"**📊 Total :** `{len(df_mouvs_fresh)}`")
    ...
    if not df_mouvs_fresh.empty and 'Type_Statut' in df_mouvs_fresh.columns and MOUV_KEY_COLUMN in df_mouvs_fresh.columns:
        ...
        if df_reservations.empty:
            st.sidebar.warning("⚠️ Aucune résa. Affichage de TOUS.")
    ...
    if not df_reservations.empty and MOUV_KEY_COLUMN in df_reservations.columns:
        st.sidebar.success(f"✅ **{len(df_reservations)}** à transformer")
        liste_resa = []
        key_map_resa = {}
        ...
        if liste_resa:
            with st.sidebar.form("form_transform"):
                resa_selectionnee = st.selectbox("Choisir", liste_resa)
                if resa_selectionnee:
                    ...
                    if key_resa:
                        ...
                        st.markdown("**📝 Détails :**")
                        st.write(f"🔑 `{key_resa}`")
                        st.write(f"🚗 {safe_str(row_resa.get('Matricule', 'N/A'))}")
                        st.write(f"👤 {safe_str(row_resa.get('Client', 'N/A'))}")
                        st.write(f"📋 {safe_str(row_resa.get('Type_Statut', 'N/A'))}")
                        st.write(f"📅 {safe_str(row_resa.get('Date_Debut', ''))} → {safe_str(row_resa.get('Date_Fin', ''))}")
                        st.write(f"💰 {safe_float(row_resa.get('Prix', 0)):,.2f} DT")
                        st.markdown("---")
                confirmer_transformation = st.checkbox("⚠️ Confirmer (irréversible)")
                if st.form_submit_button("🔄 TRANSFORMER", use_container_width=True, type="primary"):
                    ...
                    else: st.warning("⚠️ Cochez la case")
        else: st.sidebar.error("❌ Aucun")
    else: st.sidebar.error("🔴 Problème")
...
with st.container(border=True):
    ...
    with c_search1: search_date_debut = st.date_input("📅 Sortie", datetime.now(), key="adv_search_start")
    with c_search2: search_date_fin = st.date_input("📅 Retour", datetime.now() + timedelta(days=3), key="adv_search_end")
    with c_search3:
        st.markdown("<div style='height:28px;'></div>", unsafe_allow_html=True)
        btn_recherche_dispo = st.button("🔍 Vérifier", use_container_width=True)
...
with tab_planning:
    ...
    with f_col_car:
        options_recherche_voiture = ["-- Toutes --"] + liste_vehicules_opt
        vehicule_recherche = st.selectbox("🚘 Véhicule", options_recherche_voiture)
    with f_col_date_start: date_base = st.date_input("Début grille", datetime(2026, 1, 1), key="grid_bbnh_date")
    ...
    if not df_voitures_valides.empty:
        build_matrix = []
        voiture_filter = vehicule_recherche != "-- Toutes --"
        ...
        if build_matrix:
                ...
                for _, mv in df_mouvs.iterrows():
                    ...
                    while current_day <= d_fin_mv:
                        ...
                        if not (suivi_jours[m_v][key_day]["depart"] and suivi_jours[m_v][key_day]["fin"]):
                            if "garage" in s_v or "maintenance" in s_v: suivi_jours[m_v][key_day]["desc"] = f"🛠️ {client_v}"
            ...
            for idx, row in df_final_grid.iterrows():
                mat_extracted = row["Flotte"].split("[")[-1].replace("]", "").strip()
            ...
            def style_bbnh_theme(val):
                val_str = str(val)
                if "●" in val_str: return "background-color: #ffffff; color: #111827; font-size: 10px; font-weight: 600; text-align: center; border: 1px solid #e5e7eb;"
                elif "🔵" in val_str: return "background-color: #1d4ed8; color: #ffffff; font-weight: 700; font-size: 9px; border: 2px solid #60a5fa;"
                elif "🛠️" in val_str: return "background-color: #eab308; color: #1e1b4b; font-weight: 700; font-size: 10px;"
                elif "🔴" in val_str: return "background-color: #dc2626; color: #ffffff; font-weight: 600; font-size: 10px;"
                elif "🟢" in val_str: return "background-color: #16a34a; color: #ffffff; font-weight: 600; font-size: 10px;"
                return "background-color: #090b0e; color: #ffffff; font-weight: 700; font-size: 11px; border-right: 3px solid #e60000;"
            target_col_str = recherche_date.strftime("%d/%m")
            cols_ordonnees = ['Flotte']
            ...
            styled_df = style_apply(styled_df, style_bbnh_theme, subset=[c for c in cols_ordonnees if c != 'Flotte'])
...
with tab_logistique:
    ...
    if not df_actifs.empty and MOUV_KEY_COLUMN in df_actifs.columns:
        ...
        if choix_actifs:
            ...
            with col_list:
                target_v = st.selectbox("Véhicule rentrant", choix_actifs)
                d_reel = st.date_input("Date retour", datetime.now())
                t_reel = st.time_input("Heure retour", datetime.now().time())
                l_retour = st.text_input("Lieu retour", value="Siège Monastir")
                ...
                km_fin = st.number_input("KM Retour", min_value=km_dep_de_base, value=km_dep_de_base, step=1)
                if st.button("✅ VALIDER RETOUR", use_container_width=True):
                    if not key_mouv_temp: st.error("❌ Invalide")
                    else:
                        try:
            ...
            with col_details:
                if not res_dep.empty:
                    ...
                    st.markdown(f"**📊 Distance :** <span style='color:#4ade80; font-weight:bold; font-size:20px;'>{diff_km} KM</span>", unsafe_allow_html=True)
                    st.write(f"**Reste :** {row_sel.get('Reste', '0')} DT")
        else: st.info("Aucun en cours")
    else: st.info("Aucun en cours")

# --- TAB 3 : PERFORMANCE ---
with tab_analytics:
    st.markdown("### 📊 Stats du Jour")
    day_target = st.date_input("Journée", datetime.now())
    if not df_mouvs.empty:
        ...
        with col_gauche:
            ...
            if not sorties.empty:
                ...
                sorties_final = sorties[cols].rename(columns={'Matricule': '🚘 Mat', 'Client': '👤 Client', 'Type_Statut': '📋 Type', 'Date_Debut': '📅 Début', 'Date_Fin': '📅 Fin', 'Prix': '💰 Prix', 'KM_Debut': '🔢 KM'})
        ...
        with col_droite:
            ...
            if not entrees.empty:
                ...
                entrees_final = entrees_c[cols].rename(columns={'Matricule': '🚘 Mat', 'Client': '👤 Client', 'Type_Statut': '📋 Type', 'Date_Fin': '📅 Retour', 'Heure_Retour_Propre': '🕒 Heure', 'Lieu_Reception': '📍 Lieu', 'Prix': '💰 Prix', 'KM Roulé': '🔥 KM'})
...
with tab_vidange:
    ...
    if not df_vidanges.empty:
        ...
        if not alertes_critiques.empty: st.error(f"⚠️ **ALERTE :** {len(alertes_critiques)} véhicule(s)")
        else: st.success("✅ OK")
        ...
        df_tableau_affichage = df_v[cols_aff].rename(columns={'Date_Mise_A_Jour': 'MAJ', 'Marque': 'MARQUE', 'Matricule': 'MAT', 'Date_Dernier_Vidange': 'DERN.VID', 'KM_Dernier_Vidange': 'KM DERN', 'KM_Recent': 'KM REC', 'KM cerculer': 'KM CIRC', 'km restant': 'RESTE'})
        ...
        with st.container(border=True):
            if 'Matricule' in df_v.columns and not df_v.empty:
                ...
                with c_v1:
                    ...
                    date_dernier_manuel = st.date_input("Date Vidange", value=init_date_dernier)
                with c_v2:
                    dernier_km_vidange_input = st.number_input("KM Vidange", min_value=0, value=safe_int(v_info.get('KM_Dernier_Vidange')), step=1)
                    nouveau_km_actuel = st.number_input("KM Actuel", min_value=0, value=safe_int(v_info.get('KM_Recent')), step=1)
                with c_v3:
                    date_effective = st.date_input("Date opération", datetime.now())
                    action_sync = st.checkbox("Vidange aujourd'hui (reset KM)", value=False)
...
with tab_visite_tech:
    st.markdown("### 🔍 Visites Techniques")
    st.info("💡 Les **jours restants** sont calculés automatiquement")
    ...
    if not liste_flotte: st.warning("⚠️ Aucun véhicule")
    else:
        tableau_data = []
        today = datetime.now().date()
        for v in liste_flotte:
            ...
            tableau_data.append({
                "🚗 Voiture": voiture, "🔢 Mat": mat,
                "📅 1ère": date_prem, "📅 Proch": date_proch,
                "⏳ Jours": jours_restants if jours_restants is not None else "—",
                "🚨 Alerte": alerte_html
            })
        df_tableau = pd.DataFrame(tableau_data)
        alertes_urgentes = df_tableau[df_tableau["⏳ Jours"].apply(lambda x: isinstance(x, int) and x <= 15)]
        if not alertes_urgentes.empty:
            st.markdown("---")
            st.markdown('<div class="alerte-urgent">⚠️ <strong>ALERTE :</strong> ' + str(len(alertes_urgentes)) + ' véhicule(s) < 15j !</div>', unsafe_allow_html=True)
            for _, a in alertes_urgentes.iterrows():
                ...
                if isinstance(jours, int):
                    if jours < 0:
                        ...
                        msg = f"🔴 **{voiture}** [{mat}] — ÉCHU {-jours}j"
                    elif jours == 0:
                        classe = "alerte-urgent"
                        msg = f"🔴 **{voiture}** [{mat}] — AUJOURD'HUI"
                    else:
                        classe = "alerte-warning"
                        msg = f"🟠 **{voiture}** [{mat}] — {jours}j"
                    st.markdown(f'<div class="{classe}">{msg}</div>', unsafe_allow_html=True)
            st.markdown("---")
        else: st.success("✅ Aucune urgence < 15j")
        st.markdown("#### 📋 Tableau")
        edited_df = st.data_editor(
            df_tableau,
            column_config={
                "🚗 Voiture": st.column_config.TextColumn("🚗 Voiture", disabled=True, width="small"),
                "🔢 Mat": st.column_config.TextColumn("🔢 Mat", disabled=True, width="small"),
                "📅 1ère": st.column_config.DateColumn("📅 1ère", format="DD/MM/YYYY", width="small"),
                "📅 Proch": st.column_config.DateColumn("📅 Proch", format="DD/MM/YYYY", width="small"),
                "⏳ Jours": st.column_config.TextColumn("⏳ Jours", disabled=True, width="small"),
                "🚨 Alerte": st.column_config.TextColumn("🚨 Alerte", disabled=True, width="medium"),
            },
            hide_index=True, use_container_width=True,
            disabled=["🚗 Voiture", "🔢 Mat", "⏳ Jours", "🚨 Alerte"],
        ...
        with col_save:
            if st.button("💾 SAUVER", use_container_width=True, type="primary"):
                ...
                if count_saved > 0 or count_created > 0:
                    msg = ""
                    if count_saved > 0: msg += f"✅ {count_saved} MAJ"
                    if count_created > 0:
                        if msg: msg += " | "
                        msg += f"✅ {count_created} créé(s)"
                    st.success(msg)
                    if errors: st.warning(f"⚠️ Erreurs : {', '.join(errors)}")
                    get_all_tables.clear()
                    st.session_state["force_reload"] = True
                    rerun()
                else:
                    if errors: st.error(f"❌ Erreurs : {', '.join(errors)}")
                    else: st.info("ℹ️ Aucune modif")
        with col_recalc2:
        ...
        with col_l1: st.markdown('<div class="jours-restants-urgent">🔴 < 15j</div>', unsafe_allow_html=True)
        with col_l2: st.markdown('<div class="jours-restants-warning">🟠 < 30j</div>', unsafe_allow_html=True)
        with col_l3: st.markdown('<div class="jours-restants-ok">✅ > 30j</div>', unsafe_allow_html=True)
        with col_l4: st.markdown("**⚠️ Alerte** < 15j")
...
with tab_crm:
    ...
    with c1:
        ...
        search_query = st.text_input("Nom, Prénom, CIN", key="crm_search_field")
            ...
            if not clients_trouves.empty:
                for idx, cli in clients_trouves.iterrows():
                    ...
                    with st.expander(f"👤 {safe_str(cli.get('Nom')).upper()} {safe_str(cli.get('Prénom'))} (CIN: {cin_client_actuel})", expanded=True):
                        st.write(f"**📞 Tél :** `{cli.get('Numéro de téléphone', 'N/A')}` | **🚗 Permis :** `{cli.get('N° Permis', 'N/A')}`")
                        ...
                        with col_btn_mod:
                            if st.button(f"✏️ MODIFIER", key=f"btn_edit_{unique_suffix}", use_container_width=True):
                                st.session_state[f"mode_edition_{unique_suffix}"] = True
                                rerun()
                        with col_btn_sup:
                            check_sup = st.checkbox("Confirmer", key=f"chk_del_{unique_suffix}")
                            if st.button(f"🗑️ SUPPRIMER", key=f"btn_del_{unique_suffix}", use_container_width=True):
                                if check_sup:
                                    ...
                                    st.session_state["force_reload"] = True
                                    rerun()
                                else: st.warning("Cochez la case")
                        if st.session_state.get(f"mode_edition_{unique_suffix}", False):
                            ...
                            with st.form(key=f"form_reel_edit_{unique_suffix}"):
                                ...
                                e_tel = st.text_input("Tél", value=safe_str(cli.get('Numéro de téléphone')))
                                e_permis = st.text_input("Permis", value=safe_str(cli.get('N° Permis')))
                                ...
                                e_d_cin = st.date_input("Date CIN", value=e_init_d_cin)
                                e_d_per = st.date_input("Date Permis", value=e_init_d_per)
                                f_cin_remplace = st.file_uploader("Image CIN", type=["png", "jpg", "jpeg"], key=f"file_cin_{unique_suffix}")
                                f_per_remplace = st.file_uploader("Image Permis", type=["png", "jpg", "jpeg"], key=f"file_per_{unique_suffix}")
                                if st.form_submit_button("✅ SAUVER", use_container_width=True):
...
with tab_admin:
    ...
    with col_a1:
        if st.button("🗑️ PURGER MOUVEMENTS", use_container_width=True):
            if st.checkbox("Confirmer", key="chk_purge_mouv"):
                ...
                rerun()
    with col_a2:
        if st.button("🗑️ PURGER CLIENTS", use_container_width=True):
