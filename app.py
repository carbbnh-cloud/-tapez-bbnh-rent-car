...
# --- CONFIGURATION FIXE DES IDENTIFIANTS SUPABASE ---
SUPABASE_URL = "https://pwsxxmmlscvazaictocg.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB3c3h4bW1sc2N2YXphaWN0b2NnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE2ODk2MzUsImV4cCI6MjA5NzI2NTYzNX0.Dhg-fnZ_OMkk59e9w58X6DzZRr-Y3nd8PBq_cc9SH48"
...
st.markdown("""
    ...
    .km-green { background-color: #28a745; }
    .km-red { background-color: #dc3545; }
    </style>
""", unsafe_allow_html=True)

# --- CLIENT API REST SUPABASE NATIVE DE SECOURS ---
HEADERS = {
    "apikey": SUPABASE_KEY,
...
def query_supabase(method, table, json_data=None, params=None):
    url = f"{SUPABASE_URL.rstrip('/')}/rest/v1/{table}"
    try:
        with httpx.Client() as client:
            if method.upper() == "GET": response = client.get(url, headers=HEADERS, params=params)
            elif method.upper() == "POST": response = client.post(url, headers=HEADERS, json=json_data)
            elif method.upper() == "PATCH": response = client.patch(url, headers=HEADERS, json=json_data)
            elif method.upper() == "DELETE": response = client.delete(url, headers=HEADERS, params=params)
            
            if response.status_code in [200, 201, 204]:
                return response.json() if response.text else []
            return []
    except Exception as e:
        st.error(f"Erreur API Supabase : {e}")
        return []
...
def fetch_table_df(table_name):
    data = query_supabase("GET", table_name)
    return pd.DataFrame(data) if data else pd.DataFrame()

# --- CHARGEMENT SYNCHRONE DES DONNÉES CLOUD ---
df_voitures = fetch_table_df("stock")
df_c_list = fetch_table_df("clients")
df_vidanges_all = fetch_table_df("vidanges")
df_mouvs = fetch_table_df("mouvements")

liste_clients_opt = ["-- Entrée manuelle --"] + [f"{str(row.get('Nom','')).upper()} {str(row.get('Prénom',''))} (CIN: {row.get('CIN','')})" for _, row in df_c_list.iterrows()] if not df_c_list.empty else ["-- Entrée manuelle --"]
liste_vehicules_opt = [f"{str(row.get('Matricule','')).strip()}" for _, row in df_voitures.iterrows() if row.get('Matricule')] if not df_voitures.empty else []
liste_vehicules_complets_opt = [f"{str(row.get('Matricule','')).strip()} — {str(row.get('Modèle', row.get('Marque', 'Voiture')))}" for _, row in df_voitures.iterrows() if row.get('Matricule')] if not df_voitures.empty else []

# Auto-alimentation automatique de la table de vidange si nécessaire
if not df_voitures.empty:
    for _, car in df_voitures.iterrows():
        ...
        if mat and mat.lower() != 'nan':
            ...
            if not exists:
                query_supabase("POST", "vidanges", json_data={
                    "Matricule": mat, 
                    "Marque": marq, 
...
if menu_action == "📝 Nouveau Contrat / Réservation":
    ...
    if st.sidebar.button("⚡ ENREGISTRER SUR SUPABASE"):
        ...
        if client_b == "-- Entrée manuelle --":
            query_supabase("POST", "clients", json_data={
        ...
        query_supabase("POST", "mouvements", json_data={
        ...
        # Mise à jour du KM récent dans la table vidanges
        query_supabase("PATCH", "vidanges", json_data={"KM_Recent": int(km_debut), "Date_Mise_A_Jour": str_d1}, params={"Matricule": f"eq.{vehicule}"})
        st.success("Fiche synchronisée avec succès dans le Cloud Supabase !")
        st.rerun()
    ...
    with st.sidebar.form("form_add_car_supabase"):
        ...
        if st.form_submit_button("⚡ ENREGISTRER LE VÉHICULE"):
            if nouveau_matricule and nouvelle_marque and nouveau_modele:
                ...
                query_supabase("POST", "stock", json_data={
                ...
                st.success("Véhicule inséré dans la flotte globale Supabase !")
                st.rerun()
        ...
        with st.sidebar.form("form_delete_car_cloud"):
            ...
            if st.form_submit_button("💥 SUPPRIMER LE VÉHICULE"):
                if confirmer_suppression:
                    ...
                    query_supabase("DELETE", "stock", params={"Matricule": f"eq.{matricule_pure}"})
                    query_supabase("DELETE", "vidanges", params={"Matricule": f"eq.{matricule_pure}"})
    ...
    if not df_mouvs.empty:
        ...
        if not df_mouv_actifs.empty:
            ...
            if st.sidebar.button("💾 ENREGISTRER DANS LE CLOUD"):
                query_supabase("PATCH", "mouvements", json_data={
                    "Client": mod_client, "Prix": str(mod_prix), "Caution": str(mod_caution), "Reste": str(mod_reste)
                }, params={"ID": f"eq.{id_to_edit}"})
                st.success("Dossier mis à jour globalement sur Supabase !")
                st.rerun()

elif menu_action == "❌ Supprimer une opération":
    if not df_mouvs.empty:
        liste_mouv_del = [f"ID: {r['ID']} | {r['Matricule']} — {r['Client']}" for _, r in df_mouvs.iterrows()]
        with st.sidebar.form("form_delete_mouv_cloud"):
            ...
            if st.form_submit_button("💥 RETIRER DE SUPABASE"):
                if confirmer_action:
                    ...
                    query_supabase("DELETE", "mouvements", params={"ID": f"eq.{id_to_delete}"})
                    st.success("Opération effacée de la base de données !")
                    st.rerun()
    ...
    if not df_mouvs.empty:
        ...
        if not df_encours.empty:
            ...
            with st.form("form_cloture_cloud"):
                ...
                if st.form_submit_button("🏁 VALIDER LE RETOUR ET CONCLURE"):
                    query_supabase("PATCH", "mouvements", json_data={"Statut_Mouvement": "Clôturé", "KM_Fin": int(km_retour)}, params={"ID": f"eq.{id_retour}"})
                    query_supabase("PATCH", "vidanges", json_data={"KM_Recent": int(km_retour), "Date_Mise_A_Jour": datetime.now().strftime("%Y-%m-%d")}, params={"Matricule": f"eq.{str(row_ret['Matricule']).strip()}"})
        ...
        for _, cli in df_c_list.iterrows():
            with st.expander(f"👤 {str(cli.get('Nom')).upper()} - CIN: {cli.get('CIN')}"):
                ...
                if cli.get('Image CIN'):
                    try: 
                        st.image(base64.b64decode(cli['Image CIN']), width=400, caption="Scan de la pièce d'identité")
                    except Exception: 
                        pass
...
with tab_admin:
    ...
    if st.button("🗑️ PURGER TOUS LES MOUVEMENTS"):
        query_supabase("DELETE", "mouvements")
        st.success("Toutes les lignes de mouvements ont été nettoyées sur le serveur !")
        st.rerun()
