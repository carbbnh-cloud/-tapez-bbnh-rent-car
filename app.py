...
def get_data(table_name):
    try:
        ...
        st.error(f"Erreur lecture {table_name}: {e}")
        return pd.DataFrame()
...
def update_data(table_name, data_dict, filters):
    try:
        query = supabase.table(table_name).update(data_dict)
...
df_voitures = get_data(T_VEHICULE)
df_clients = get_data(T_CLIENT)
df_mouvs = get_data(T_MOUVEMENT)
df_vidanges = get_data(T_VIDANGE)
df_contrats = get_data(T_CONTRAT)
        ...
        if f_clients:
            try:
                ...
                for _, row in df_cli.iterrows():
                    insert_data(T_CLIENT, row.to_dict())
        ...
        if f_loc2:
            try:
                ...
                for _, row in df_stock.iterrows():
                    insert_data(T_VEHICULE, row.to_dict())
                ...
                for _, row in df_mouv_raw.iterrows():
                    ...
                    data_mouv = {
                        ...
                        "Date_Debut": str(row.get('Date_Debut', '')), "Heure_Debut": h_d, "Date_Fin": str(row.get('Date_Fin', '')), "Heure_Fin": h_f,
                        "Client": str(row.get('Client', 'Client')), "Prix": p_clean, "Statut_Mouvement": 'En cours',
                        "Caution": p_clean, "Reste": p_clean, "Lieu_Reception": str(row.get('Lieu_Reception', 'Siège')),
                        "KM_Debut": km_d, "KM_Fin": km_f
                    }
                    insert_data(T_MOUVEMENT, data_mouv)
                st.success("Données intégrées avec succès !")
                st.rerun()
            except Exception as e: st.error(f"Erreur : {e}")

# ========================================================================
# LOGIQUE SIDEBAR (FORMULAIRES)
# ========================================================================
if menu_action == "📝 Nouveau Contrat / Réservation":
    ...
    if st.sidebar.button("⚡ ENREGISTRER ON THE PLANNING"):
        ...
        update_data(T_VIDANGE, {"KM_Recent": int(km_debut), "Date_Mise_A_Jour": str_d1}, {"Matricule": vehicule})
    ...
    with st.sidebar.form("form_bbnh_add_car"):
        ...
        if st.form_submit_button("⚡ ENREGISTRER LE VEHICULE"):
            if nouveau_matricule and nouvelle_marque and nouveau_modele:
                insert_data(T_VEHICULE, {
                    "Matricule": nouveau_matricule, "Marque": nouvelle_marque, "Modèle": nouveau_modele,
                    "Année": nouvelle_annee, "Marque/Model": f"{nouvelle_marque} {nouveau_modele}"
                })
                insert_data(T_VIDANGE, {
        ...
        with st.sidebar.form("form_bbnh_delete_car"):
            ...
            if st.form_submit_button("💥 SUPPRIMER LE VÉHICULE"):
                if confirmer_suppression:
                    ...
                    delete_data(T_VEHICULE, {"Matricule": matricule_pure})
                    delete_data(T_VIDANGE, {"Matricule": matricule_pure})
    ...
    if not df_mouv_actifs.empty:
        ...
        selected_idx = int(mouv_selectionne.split(" | ")[0].replace("ID: ", "").strip())
        row_init = df_mouv_actifs.iloc[selected_idx] if selected_idx < len(df_mouv_actifs) else df_mouv_actifs.iloc[0]
        ...
        st.sidebar.markdown(f"### ⚙️ Édition Totale du Dossier #{selected_idx}")
        ...
        mod_prix = st.sidebar.number_input("Prix Total Évalué (DT) : ", value=int(mod_nbr_jours * mod_prix_unitaire), key="mod_tot")
        ...
        if st.sidebar.button("💾 ENREGISTRER ABSOLUMENT TOUT"):
            ...
            update_data(T_CLIENT, {
                "Nom": mod_client, "N° Permis": mod_permis
            }, {"CIN": mod_cin})
            
            update_data(T_MOUVEMENT, {
            ...
            update_data(T_VIDANGE, {"KM_Recent": int(mod_km_deb), "Date_Mise_A_Jour": str_mod_d1}, {"Matricule": mod_vehicule})
        ...
        with st.sidebar.form("form_bbnh_delete_mouv"):
            ...
            if st.form_submit_button("💥 RETIRER DU PLANNING"):
                if confirmer_action:
                    ...
                    delete_data(T_MOUVEMENT, {"id": id_to_delete})
...
with tab_contrats:
    ...
    if not df_mouvs.empty:
        ...
        for _, row in df_mouvs.iterrows():
            matricule = str(row.get('Matricule', ''))
            client = str(row.get('Client', ''))
            tel = "N/A"
            if not df_clients.empty:
                df_tel = df_clients[df_clients['Nom'] == client]
                if not df_tel.empty: tel = str(df_tel.iloc[0].get('Numéro de téléphone', 'N/A'))
            ...
            try:
                ...
                if jours <= 0: jours = 1
            except: d_dep, d_ret, jours = str(row.get('Date_Debut', '')), str(row.get('Date_Fin', '')), "?"
            
            h_dep, h_ret = str(row.get('Heure_Debut', '00:00')), str(row.get('Heure_Fin', '00:00'))
...
with tab_logistique:
    ...
    if not df_actifs.empty:
        ...
        with col_list:
            ...
            km_dep_de_base = 0
            if id_mouv_temp > 0:
                res_dep = df_actifs[df_actifs['id'] == id_mouv_temp] if 'id' in df_actifs.columns else df_actifs.iloc[[0]]
            ...
            if st.button("✅ VALIDATION DU RETOUR", use_container_width=True):
                ...
                update_data(T_MOUVEMENT, {"Statut_Mouvement": "Retourné", "Date_Fin": d_reel.strftime("%Y-%m-%d"), "Heure_Fin": str_t_reel, "Lieu_Reception": l_retour, "KM_Fin": int(km_fin)}, {"id": int(id_mouv)})
                update_data(T_VIDANGE, {"KM_Recent": int(km_fin), "Date_Mise_A_Jour": d_reel.strftime("%Y-%m-%d")}, {"Matricule": vehicule_rentre})
    ...
    if not df_vidanges.empty:
        ...
        with st.container(border=True):
            ...
            if st.button(" ENREGISTRER ET RECALCULER DIRECTEMENT", use_container_width=True):
                ...
                if action_sync:
                    update_data(T_VIDANGE, {"KM_Recent": int(nouveau_km_actuel), "KM_Dernier_Vidange": int(nouveau_km_actuel), "Date_Dernier_Vidange": date_operation_str, "Date_Mise_A_Jour": date_operation_str}, {"Matricule": v_select})
                else:
                    update_data(T_VIDANGE, {"KM_Recent": int(nouveau_km_actuel), "KM_Dernier_Vidange": int(dernier_km_vidange_input), "Date_Dernier_Vidange": date_historique_str, "Date_Mise_A_Jour": date_operation_str}, {"Matricule": v_select})
    ...
    with c1:
        ...
        if search_query:
            clients_trouves = df_clients[
                (df_clients['Nom'].astype(str).str.contains(search_query, case=False, na=False)) |
                (df_clients['Prénom'].astype(str).str.contains(search_query, case=False, na=False)) |
                (df_clients['CIN'].astype(str).str.contains(search_query, case=False, na=False))
                    ...
                    with st.expander(f"👤 {str(cli.get('Nom', '')).upper()} {cli.get('Prénom', '')} (CIN: {cin_client_actuel})", expanded=True):
                        ...
                        with col_btn_sup:
                            ...
                            if st.button(f"🗑️ SUPPRIMER CE CLIENT", key=f"btn_del_{unique_suffix}"):
                                if check_sup:
                                    delete_data(T_CLIENT, {"CIN": cin_client_actuel})
                        ...
                        if st.session_state.get(f"mode_edition_{unique_suffix}", False):
                            ...
                            with st.form(key=f"form_reel_edit_{unique_suffix}"):
                                ...
                                if st.form_submit_button("✅ METTRE À JOUR"):
                                    client_update = {"Prénom": e_prenom, "Nom": e_nom, "Numéro de téléphone": e_tel, "N° Permis": e_permis, "Date Délivrance CIN": e_d_cin.strftime("%Y-%m-%d"), "Date Délivrance Permis": e_d_per.strftime("%Y-%m-%d")}
                                    if f_cin_remplace: client_update["Image CIN"] = encoder_image_base64(f_cin_remplace)
                                    if f_per_remplace: client_update["Image Permis"] = encoder_image_base64(f_per_remplace)
                                    update_data(T_CLIENT, client_update, {"CIN": cin_client_actuel})
        ...
        with st.form("form_new_client_crm"):
            ...
            if st.form_submit_button("⚡ CRÉER LE PROFIL CLIENT"):
                if n_prenom and n_nom and n_cin:
                    insert_data(T_CLIENT, {"Prénom": n_prenom, "Nom": n_nom, "CIN": n_cin, "Numéro de téléphone": n_tel, "N° Permis": n_permis, "Date Délivrance CIN": n_d_cin.strftime("%Y-%m-%d"), "Date Délivrance Permis": n_d_per.strftime("%Y-%m-%d"), "Image CIN": encoder_image_base64(f_cin_new), "Image Permis": encoder_image_base64(f_per_new)})
    ...
    with col_a1:
        if st.button("🗑️ PURGER TOUS LES MOUVEMENTS"):
            if st.checkbox("Confirmer la purge des mouvements"):
                supabase.table(T_MOUVEMENT).delete().neq("id", 0).execute()
    ...
    with col_a2:
        if st.button("🗑️ RÉINITIALISER LA BASE CLIENTS"):
            if st.checkbox("Confirmer la purge des clients"):
                supabase.table(T_CLIENT).delete().neq("CIN", "").execute()
