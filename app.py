...
st.markdown("""
    ...
    div[data-testid="stRadio"] label {
        font-size: 15px !important; font-weight: 500 !important; padding: 8px 4px !important;
    }
    .stTabs [data-baseweb="tab-list"] {
        gap: 8px; background-color: #161920; padding: 6px; border-radius: 14px;
        border: 1px solid #222733; margin-bottom: 25px;
    }
    .stTabs [data-baseweb="tab"] {
        padding: 10px 20px; border-radius: 10px; font-weight: 600; color: #9ca3af;
    ...
    div[data-testid="stForm"] {
        background: rgba(22,25,32,0.8) !important; border: 1px solid #2a3142 !important;
        padding: 25px !important; border-radius: 16px !important;
        box-shadow: 0 4px 20px rgba(0,0,0,0.2) !important;
    }
    input, select, textarea, div[data-baseweb="select"] {
        background-color: #1a1e26 !important; color: #ffffff !important;
        border: 1px solid #2a3142 !important; border-radius: 10px !important; font-size: 14px !important;
    }
    div.stButton > button {
        background: linear-gradient(135deg, #e60000 0%, #b30000 100%) !important;
        color: white !important; border: none !important; border-radius: 10px !important;
        padding: 14px 28px !important; font-weight: 700 !important; letter-spacing: 0.5px;
    ...
    .contract-table {
        width: 100%; border-collapse: collapse; background-color: #ffffff; color: #333333;
        border-radius: 8px; overflow: hidden; font-size: 13px;
    }
    .contract-table th {
        background-color: #f8f9fa; color: #666; font-weight: 600; text-align: center;
        padding: 12px 8px; border-bottom: 1px solid #eee;
    }
    .contract-table td {
        padding: 12px 8px; border-bottom: 1px solid #eee; text-align: center; vertical-align: middle;
    }
    ...
    .km-indicator {
        width: 80px; padding: 2px 4px; color: white; font-size: 10px;
...
# - Cache de connexion pour éviter d'ouvrir une nouvelle socket à chaque rerun
# =========================================================================
...
def get_db_connection():
    ...
    if not dsn:
        st.error("❌ Aucune connexion Supabase configurée.")
        st.info(
            "Ajoutez ceci à `.streamlit/secrets.toml` :\n\n"
            "```toml\n"
            "[supabase]\n"
            'connection_string = "postgresql://postgres:VOTRE_MOT_DE_PASSE@db.xxxx.supabase.co:5432/postgres"\n'
            "```\n\n"
            "Ou exportez la variable d'environnement `SUPABASE_DB_URL`."
        )
        st.stop()
...
def ping_connection(_conn):
    """Vérifie que la connexion est vivante, sinon la recrée via le cache."""
    try:
...
# --- HELPERS D'INTROSPECTION DE SCHÉMA (équivalent de PRAGMA table_info) ---
def _col_exists(cur, table_name, column_name):
    cur.execute(
    ...
    with conn.cursor() as cur:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS clients (
                ...
                "Lieu & Date Naissance" TEXT,
                "N° Permis" TEXT,
                "Date Délivrance Permis" TEXT,
                "Adresse" TEXT,
                "Remarque" TEXT,
                "Numéro de téléphone" TEXT,
                "Image CIN" TEXT DEFAULT '',
                "Image Permis" TEXT DEFAULT ''
            )
        """)
        colonnes_a_ajouter_clients = {
            "Image CIN": "TEXT DEFAULT ''",
            "Image Permis": "TEXT DEFAULT ''",
            "Date Délivrance CIN": "TEXT DEFAULT ''",
            "Date Délivrance Permis": "TEXT DEFAULT ''",
        }
        for col_name, col_type in colonnes_a_ajouter_clients.items():
            if not _col_exists(cur, "clients", col_name):
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS vidanges (
                ...
                "Date_Dernier_Vidange" TEXT,
                "KM_Dernier_Vidange" INTEGER DEFAULT 0,
        ...
        # Table mouvements (avec migrations)
        if not _col_exists(cur, "mouvements", "ID"):
            cur.execute("DROP TABLE IF EXISTS mouvements")
            cur.execute("""
                CREATE TABLE mouvements (
                    ...
                    "Prix" TEXT DEFAULT '0',
                    "CHEV" TEXT DEFAULT '0',
                    "Statut_Mouvement" TEXT DEFAULT 'En cours',
            ...
            for col_name, col_type in nouvelles_cols.items():
                if not _col_exists(cur, "mouvements", col_name):
                    cur.execute(f'ALTER TABLE mouvements ADD COLUMN "{col_name}" {col_type}')

        # Table contrats
        cur.execute("""
            CREATE TABLE IF NOT EXISTS contrats (
                "Num_Contrat" TEXT PRIMARY KEY,
                "Matricule" TEXT,
                "Client_Nom" TEXT,
                "CIN_Client" TEXT,
                "Date_Debut" TEXT,
...
def executer(sql, params=(), modifier=False):
    ...
    reussi = True
    try:
        # Nettoie un éventuel état de transaction avortée d'un appel précédent
        conn.rollback()
        if modifier:
            with conn.cursor() as cur:
...
def bulk_replace_table(_conn, df, table_name):
    """Remplace le contenu d'une table par les lignes d'un DataFrame
    (équivalent d'un df.to_sql(..., if_exists='replace') sur Postgres).
    Préserve le schéma existant, ne touche pas les autres tables."""
    if df.empty:
        return
    ...
    with _conn.cursor() as cur:
        ...
        rows = [
            tuple(None if pd.isna(v) else v for v in row_vals)
...
# =========================================================================
df_voitures = executer('SELECT * FROM stock ORDER BY "Marque"')
df_c_list = executer('SELECT "Nom", "Prénom", "CIN" FROM clients')

liste_clients_opt = (
    ["-- Entrée manuelle --"]
    + [
        f"{str(row['Nom']).upper()} {str(row['Prénom'])} (CIN: {row['CIN']})"
        for _, row in df_c_list.iterrows()
    ]
...
liste_vehicules_opt = (
    [
        f"{str(row['Matricule']).strip()}"
        for _, row in df_voitures.iterrows()
        if pd.notna(row['Matricule']) and str(row['Matricule']).strip().lower() != 'nan'
    ]
...
liste_vehicules_complets_opt = (
    [
        f"{str(row['Matricule']).strip()} — {str(row.get('Modèle', row.get('Marque', 'Voiture')))}"
        for _, row in df_voitures.iterrows()
        if pd.notna(row['Matricule'])
    ]
    if not df_voitures.empty
    else []
)

# Synchroniser les véhicules connus dans la table vidanges
for _, car in df_voitures.iterrows():
    mat = str(car.get('Matricule', '')).strip()
    marq = str(car.get('Marque', '')).upper()
    if mat and mat.lower() != 'nan':
        executer(
            'INSERT INTO vidanges ("Matricule", "Marque", "Date_Mise_A_Jour", '
            '"Date_Dernier_Vidange", "KM_Dernier_Vidange", "KM_Recent") '
            'VALUES (%s, %s, %s, %s, 0, 0) '
            'ON CONFLICT ("Matricule") DO NOTHING',
            (mat, marq, datetime.now().strftime("%Y-%m-%d"),
            ...
            try:
                ...
                with conn.cursor() as cur:
                    ...
                    for _, row in df_mouv_raw.iterrows():
                        ...
                        try:
                            km_d = int(float(str(row.get('KM_Debut', 0)).strip().replace(' ', '')))
                        except Exception:
                            km_d = 0
                        try:
                            km_f = int(float(str(row.get('KM_Fin', 0)).strip().replace(' ', '')))
                        except Exception:
                            km_f = 0
                        p_raw = row.get('Prix', 0)
                        try:
                            p_clean = str(float(p_raw))
                        except Exception:
                            p_clean = "0"
                        cur.execute(
                            'INSERT INTO mouvements ("Matricule", "Type_Statut", '
                            '"Date_Debut", "Heure_Debut", "Date_Fin", "Heure_Fin", '
                            '"Client", "Prix", "Statut_Mouvement", "Caution", "Reste", '
                            '"Lieu_Reception", "KM_Debut", "KM_Fin") '
                            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'En cours', '0', %s, %s, %s, %s)",
                            (
                                str(row.get('Matricule', 'Inconnu')),
                                str(row.get('Type_Statut', 'Location')),
                                str(row.get('Date_Debut', '')), h_d,
                                str(row.get('Date_Fin', '')), h_f,
                                str(row.get('Client', 'Client')),
                                p_clean, p_clean,
                                str(row.get('Lieu_Reception', 'Siège')),
                                km_d, km_f,
                            ),
                        )
                conn.commit()
...
# Chargement sécurisé des mouvements (après imports éventuels)
...
if menu_action == "📝 Nouveau Contrat / Réservation":
    ...
    if st.sidebar.button("⚡ ENREGISTRER ON THE PLANNING"):
        ...
        if client_b == "-- Entrée manuelle --":
            executer(
                'INSERT INTO clients ("Nom", "CIN", "Date Délivrance CIN", '
                '"N° Permis", "Date Délivrance Permis", "Image CIN", "Image Permis") '
                'VALUES (%s, %s, %s, %s, %s, %s, %s) '
                'ON CONFLICT ("CIN") DO UPDATE SET '
                '"Nom" = EXCLUDED."Nom", '
                '"Date Délivrance CIN" = EXCLUDED."Date Délivrance CIN", '
                '"N° Permis" = EXCLUDED."N° Permis", '
                '"Date Délivrance Permis" = EXCLUDED."Date Délivrance Permis", '
                '"Image CIN" = EXCLUDED."Image CIN", '
                '"Image Permis" = EXCLUDED."Image Permis"',
                (nom_f, cin_f, dc_m.strftime("%Y-%m-%d"), permis_m,
                 dp_m.strftime("%Y-%m-%d"), img_cin_b64, img_permis_b64),
                modifier=True,
            )

        if "Contrat" in nature:
            executer(
                'INSERT INTO contrats ("Num_Contrat", "Matricule", "Client_Nom", '
                '"CIN_Client", "Date_Debut", "Heure_Debut", "Date_Fin", "Heure_Fin", '
                '"Tarif_Jour", "Montant_Total", "Statut_Contrat") '
                "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Actif') "
                'ON CONFLICT ("Num_Contrat") DO NOTHING',
                (ref, vehicule, nom_f, cin_f, str_d1, str_t1, str_d2, str_t2,
                 str(prix_unitaire), str(montant_total)),
                modifier=True,
            )

        executer(
            'INSERT INTO mouvements ("Matricule", "Type_Statut", "Date_Debut", '
            '"Heure_Debut", "Date_Fin", "Heure_Fin", "Client", "Prix", '
            '"Statut_Mouvement", "Caution", "Reste", "Lieu_Reception", '
            '"No_Vol", "Info_Note", "KM_Debut", "KM_Fin") '
            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'En cours', %s, %s, %s, %s, %s, %s, 0)",
            (vehicule, text_type, str_d1, str_t1, str_d2, str_t2, nom_f,
             str(montant_total), str(caution), str(reste), l_reception,
             no_vol, info_note, int(km_debut)),
            modifier=True,
        )

        executer(
            'UPDATE vidanges SET "KM_Recent" = %s, "Date_Mise_A_Jour" = %s '
            'WHERE "Matricule" = %s',
            (int(km_debut), str_d1, vehicule),
            modifier=True,
        )
    ...
    with st.sidebar.form("form_bbnh_add_car"):
        ...
        if st.form_submit_button("⚡ ENREGISTRER LE VEHICULE"):
            if nouveau_matricule and nouvelle_marque and nouveau_modele:
                ...
                executer(
                    'INSERT INTO stock ("Matricule", "Marque", "Modèle", "Année", "Marque/Model") '
                    'VALUES (%s, %s, %s, %s, %s) '
                    'ON CONFLICT ("Matricule") DO UPDATE SET '
                    '"Marque" = EXCLUDED."Marque", '
                    '"Modèle" = EXCLUDED."Modèle", '
                    '"Année" = EXCLUDED."Année", '
                    '"Marque/Model" = EXCLUDED."Marque/Model"',
                    (nouveau_matricule, nouvelle_marque, nouveau_modele,
                     nouvelle_annee, combinaison_modele),
                    modifier=True,
                )
                executer(
                    'INSERT INTO vidanges ("Matricule", "Marque", "Date_Mise_A_Jour", "Date_Dernier_Vidange") '
                    'VALUES (%s, %s, %s, %s) '
                    'ON CONFLICT ("Matricule") DO NOTHING',
                    (nouveau_matricule, nouvelle_marque.upper(),
                     datetime.now().strftime("%Y-%m-%d"),
                     datetime.now().strftime("%Y-%m-%d")),
                    modifier=True,
                )
        ...
        with st.sidebar.form("form_bbnh_delete_car"):
            ...
            if st.form_submit_button("💥 SUPPRIMER LE VÉHICULE"):
                if confirmer_suppression:
                    ...
                    executer('DELETE FROM stock WHERE "Matricule" = %s', (matricule_pure,), modifier=True)
                    executer('DELETE FROM vidanges WHERE "Matricule" = %s', (matricule_pure,), modifier=True)
    ...
    if not df_mouv_actifs.empty:
        ...
        df_cli_spec = executer('SELECT * FROM clients WHERE "Nom" = %s', (str(row_init['Client']),))
        row_cli_init = df_cli_spec.iloc[0] if not df_cli_spec.empty else {}

        try:
            init_date_deb = datetime.strptime(str(row_init['Date_Debut']), "%Y-%m-%d").date()
        except Exception:
            init_date_deb = datetime.now().date()
        try:
            init_date_fin = datetime.strptime(str(row_init['Date_Fin']), "%Y-%m-%d").date()
        except Exception:
            init_date_fin = datetime.now().date()
        try:
            init_time_deb = datetime.strptime(formater_heure_propre(row_init['Heure_Debut']), "%H:%M").time()
        except Exception:
            init_time_deb = time(9, 0)
        try:
            init_time_fin = datetime.strptime(formater_heure_propre(row_init['Heure_Fin']), "%H:%M").time()
        except Exception:
            init_time_fin = time(12, 0)
        try:
            init_date_cin = datetime.strptime(str(row_cli_init.get('Date Délivrance CIN')), "%Y-%m-%d").date()
        except Exception:
            init_date_cin = datetime.now().date()
        try:
            init_date_permis = datetime.strptime(str(row_cli_init.get('Date Délivrance Permis')), "%Y-%m-%d").date()
        except Exception:
            init_date_permis = datetime.now().date()

        st.sidebar.markdown(f"### ⚙️ Édition Totale du Dossier #{id_to_edit}")
        mod_nature = st.sidebar.selectbox(
            "Changer de Nature :",
            ["Location", "Réservation", "Maintenance / Garage"],
            index=["location", "réservation", "maintenance / garage"].index(str(row_init['Type_Statut']).lower())
            if str(row_init['Type_Statut']).lower() in ["location", "réservation", "maintenance / garage"] else 0,
        )
        ...
        with c_d1:
            mod_d1 = st.sidebar.date_input("Date Début :", init_date_deb, key="mod_d1")
        with c_t1:
            mod_t1 = st.sidebar.time_input("Heure Début :", init_time_deb, key="mod_t1")
        c_d2, c_t2 = st.sidebar.columns(2)
        with c_d2:
            mod_d2 = st.sidebar.date_input("Date Fin :", init_date_fin, key="mod_d2")
        with c_t2:
            mod_t2 = st.sidebar.time_input("Heure Fin / Retour :", init_time_fin, key="mod_t2")
        ...
        df_contrat_spec = executer('SELECT "Tarif_Jour" FROM contrats WHERE "Num_Contrat" = %s', (str(id_to_edit),))
        init_tarif_unitaire = 100
        if not df_contrat_spec.empty:
            try:
                init_tarif_unitaire = int(float(df_contrat_spec.iloc[0]['Tarif_Jour']))
            except Exception:
                pass
        ...
        if st.sidebar.button("💾 ENREGISTRER ABSOLUMENT TOUT"):
            ...
            if mod_f_cin:
                executer(
                    'UPDATE clients SET "Image CIN" = %s WHERE "CIN" = %s',
                    (encoder_image_base64(mod_f_cin), mod_cin),
                    modifier=True,
                )
            if mod_f_permis:
                executer(
                    'UPDATE clients SET "Image Permis" = %s WHERE "CIN" = %s',
                    (encoder_image_base64(mod_f_permis), mod_cin),
                    modifier=True,
                )

            executer(
                'UPDATE clients SET "Nom" = %s, "Date Délivrance CIN" = %s, '
                '"N° Permis" = %s, "Date Délivrance Permis" = %s '
                'WHERE "CIN" = %s',
                (mod_client, mod_date_cin.strftime("%Y-%m-%d"), mod_permis,
                 mod_date_permis.strftime("%Y-%m-%d"), mod_cin),
                modifier=True,
            )

            executer(
                'UPDATE mouvements SET "Matricule" = %s, "Type_Statut" = %s, '
                '"Client" = %s, "Date_Debut" = %s, "Heure_Debut" = %s, '
                '"Date_Fin" = %s, "Heure_Fin" = %s, "Prix" = %s, "Caution" = %s, '
                '"Reste" = %s, "Lieu_Reception" = %s, "No_Vol" = %s, '
                '"Info_Note" = %s, "KM_Debut" = %s '
                'WHERE "ID" = %s',
                (
                    mod_vehicule, mod_nature, mod_client, str_mod_d1, str_mod_t1,
                    str_mod_d2, str_mod_t2, str(mod_prix), str(mod_caution),
                    str(mod_reste), mod_lieu, mod_vol, mod_note,
                    int(mod_km_deb), id_to_edit,
                ),
                modifier=True,
            )
            ...
            executer(
                'UPDATE vidanges SET "KM_Recent" = %s, "Date_Mise_A_Jour" = %s '
                'WHERE "Matricule" = %s',
                (int(mod_km_deb), str_mod_d1, mod_vehicule),
                modifier=True,
            )
        ...
        with st.sidebar.form("form_bbnh_delete_mouv"):
            ...
            if st.form_submit_button("💥 RETIRER DU PLANNING"):
                if confirmer_action:
                    ...
                    executer('DELETE FROM mouvements WHERE "ID" = %s', (id_to_delete,), modifier=True)
...
# =========================================================================
st.markdown("<h1>BBNH WORKSPACE AUTOMATION — Supabase Edition</h1>", unsafe_allow_html=True)
...
with tab_planning:
    ...
    df_voitures_valides = (
        df_voitures[df_voitures['Matricule'].notna() & (df_voitures['Matricule'].str.strip().str.lower() != 'nan')]
        if not df_voitures.empty else pd.DataFrame()
    )
    ...
    if not df_voitures_valides.empty:
        ...
        if len(build_matrix) > 0:
            ...
            st.dataframe(
                df_final_grid[cols_ordonnees].style.map(style_bbnh_theme, subset=[c for c in cols_ordonnees if c != 'Flotte BBNH']),
...
with tab_contrats:
    ...
    if df_contrats_list is not None and not df_contrats_list.empty:
        ...
        <table class="contract-table">
            <thead>
                <tr>
                    <th>Voiture</th><th>Tél</th><th>N° Contrat</th><th>Facture</th>
                    <th>D.Départ</th><th>D.Retour</th><th>Jours</th>
                    <th>Montant TTC(DT)</th><th>Reste(DT)</th><th>Extras</th>
                    <th>KM Sortie</th><th>KM Retour</th><th>KM</th>
                </tr>
            </thead>
            <tbody>
        """
        for _, row in df_contrats_list.iterrows():
            ...
            df_tel = executer('SELECT "Numéro de téléphone" FROM clients WHERE "Nom" = %s', (client,))
            tel = df_tel.iloc[0]['Numéro de téléphone'] if df_tel is not None and not df_tel.empty else "N/A"
            try:
                d_dep = datetime.strptime(row['Date_Debut'], "%Y-%m-%d").strftime("%d/%m/%Y")
                d_ret = datetime.strptime(row['Date_Fin'], "%Y-%m-%d").strftime("%d/%m/%Y")
                jours = (datetime.strptime(row['Date_Fin'], "%Y-%m-%d") - datetime.strptime(row['Date_Debut'], "%Y-%m-%d")).days
                if jours <= 0:
                    jours = 1
            except Exception:
                d_dep = row['Date_Debut']
                d_ret = row['Date_Fin']
                jours = "?"
            h_dep = row['Heure_Debut']
            h_ret = row['Heure_Fin']
            try:
                montant = f"{float(row['Prix']):,.3f}"
            except Exception:
                montant = "0.000"
            try:
                reste_val = float(row['Reste'])
            except Exception:
                reste_val = 0.0
            reste_style = "status-paid" if reste_val <= 0 else "status-pending"
            reste_text = "PAYÉ" if reste_val <= 0 else f"{reste_val:,.3f} DT"
            try:
                km_s = int(row['KM_Debut'])
            except Exception:
                km_s = 0
            try:
                km_r = int(row['KM_Fin'])
            ...
            html_table += f"""
                <tr>
                    ...
                    <td>
                        <div class="contract-num">{row['ID']}</div>
                    ...
                    <td>
                        ...
                        <div style="background:#ffff00; font-size:9px; padding:2px; font-weight:bold;">Imprimer Extrait</div>
                    </td>
                    ...
                    <td>
                        <div style="background:#f1f3f5; padding:5px; border-radius:4px; font-size:10px;">
                            <span style="color:green;">✔</span><br>
                            {float(row.get('Caution', 0)):,.3f} DT
                        </div>
                    </td>
                    <td>
                        <div class="km-box">
                            ...
                            <div class="km-indicator km-purple">{km_ess_s}</div>
                            <div class="km-indicator km-black">{km_dt_s}</div>
                    ...
                    <td>
                        <div class="km-box">
                            ...
                            <div class="km-indicator km-red">{km_j_r}</div>
                            <div class="km-indicator km-orange">{km_j_r}</div>
                            <div class="km-indicator km-black">{km_dt_r}</div>
                            <div style="color:green; font-size:12px;">✔</div>
                        </div>
                    </td>
...
with tab_logistique:
    ...
    if df_actifs is not None and not df_actifs.empty:
        ...
        with col_list:
            ...
            if id_mouv_temp > 0:
                ...
                if not res_dep.empty:
                    try:
                        km_dep_de_base = int(float(str(res_dep.iloc[0].get('KM_Debut', 0)).replace(' ', '')))
                    except Exception:
                        km_dep_de_base = 0
            ...
            if st.button("✅ VALIDATION DU RETOUR", use_container_width=True):
                ...
                executer(
                    "UPDATE mouvements SET \"Statut_Mouvement\" = 'Retourné', "
                    "\"Date_Fin\" = %s, \"Heure_Fin\" = %s, \"Lieu_Reception\" = %s, "
                    "\"KM_Fin\" = %s WHERE \"ID\" = %s",
                    (d_reel.strftime("%Y-%m-%d"), str_t_reel, l_retour, int(km_fin), id_mouv),
                    modifier=True,
                )
                executer(
                    "UPDATE vidanges SET \"KM_Recent\" = %s, \"Date_Mise_A_Jour\" = %s "
                    "WHERE \"Matricule\" = %s",
                    (int(km_fin), d_reel.strftime("%Y-%m-%d"), vehicule_rentre),
                    modifier=True,
                )
...
with tab_analytics:
    ...
    if df_mouvs is not None and not df_mouvs.empty:
        ...
        with col_gauche:
            ...
            if not sorties.empty:
                sorties_final = sorties[['Matricule', 'Client', 'Date_Debut', 'Date_Fin', 'Prix', 'KM_Debut']].rename(columns={
                    'Matricule': '🚘 Matricule', 'Client': '👤 Client / Conducteur',
                    'Date_Debut': '📅 DATE SORTIE', 'Date_Fin': '📅 DATE RETOUR PRÉVUE',
                    'Prix': '💰 PRIX (DT)', 'KM_Debut': '🔢 KM SORTIE'
                })
        ...
        with col_droite:
            ...
            if not entrees.empty:
                ...
                entrees_final = entrees[['Matricule', 'Client', 'Date_Debut', 'Date_Fin', 'Heure_Retour_Propre', 'Lieu_Reception', 'Prix', 'KM_Debut', 'KM_Fin', 'KM Roulé']].rename(columns={
                    'Matricule': '🚘 Matricule', 'Client': '👤 Client / Conducteur',
                    'Date_Debut': '📅 DATE SORTIE', 'Date_Fin': '📅 DATE RETOUR',
                    'Heure_Retour_Propre': '🕒 HEURE RETOUR', 'Lieu_Reception': '📍 LIEU DE RETOUR',
                    'Prix': '💰 PRIX TOTAL (DT)', 'KM_Debut': '🔢 KM SORTIE',
                    'KM_Fin': '🔢 KM RETOUR', 'KM Roulé': '🔥 KM ROULÉ'
                })
...
with tab_vidange:
    ...
    if df_v_base is not None and not df_v_base.empty:
        ...
        df_tableau_affichage = df_v_base[['Date_Mise_A_Jour', 'Marque', 'Matricule', 'Date_Dernier_Vidange', 'KM_Dernier_Vidange', 'KM_Recent', 'KM cerculer', 'km restant']].rename(columns={
            'Date_Mise_A_Jour': 'DATE MIS AJOURS', 'Marque': 'MARQUE',
            'Matricule': 'MATRICULE', 'Date_Dernier_Vidange': 'TE DERNIER VIDAN',
            'KM_Dernier_Vidange': 'DERNIER VIDA', 'KM_Recent': 'KM RECENT',
            'KM cerculer': 'KM cerculer', 'km restant': 'km restant'
        })
        ...
        with st.container(border=True):
            ...
            with c_v1:
                ...
                try:
                    init_date_dernier = datetime.strptime(str(v_info['Date_Dernier_Vidange']), "%Y-%m-%d").date()
                except Exception:
                    init_date_dernier = datetime.now().date()
            ...
            if st.button("💾 ENREGISTRER ET RECALCULER DIRECTEMENT", use_container_width=True):
                ...
                if action_sync:
                    executer(
                        'UPDATE vidanges SET "KM_Recent" = %s, "KM_Dernier_Vidange" = %s, '
                        '"Date_Dernier_Vidange" = %s, "Date_Mise_A_Jour" = %s '
                        'WHERE "Matricule" = %s',
                        (int(nouveau_km_actuel), int(nouveau_km_actuel), date_operation_str,
                         date_operation_str, v_select),
                        modifier=True,
                    )
                else:
                    executer(
                        ...
                        '"Date_Dernier_Vidange" = %s, "Date_Mise_A_Jour" = %s '
                        'WHERE "Matricule" = %s',
                        (int(nouveau_km_actuel), int(dernier_km_vidange_input),
                         date_historique_str, date_operation_str, v_select),
                        modifier=True,
                    )
...
with tab_crm:
    ...
    with c1:
        ...
        if search_query:
            clients_trouves = executer(
                'SELECT * FROM clients WHERE "Nom" LIKE %s OR "Prénom" LIKE %s OR "CIN" LIKE %s',
                (f"%{search_query}%", f"%{search_query}%", f"%{search_query}%"),
            )
            if clients_trouves is not None and not clients_trouves.empty:
                    ...
                    with st.expander(f"👤 {str(cli['Nom']).upper()} {cli['Prénom']} (CIN: {cin_client_actuel})", expanded=True):
                        ...
                        with col_img1:
                            if cli.get('Image CIN'):
                                try:
                                    st.image(base64.b64decode(cli['Image CIN']), caption="Pièce d'identité (CIN)", use_container_width=True)
                                except Exception:
                                    pass
                        with col_img2:
                            if cli.get('Image Permis'):
                                try:
                                    st.image(base64.b64decode(cli['Image Permis']), caption="Permis de conduire", use_container_width=True)
                                except Exception:
                                    pass
                        ...
                        with col_btn_sup:
                            ...
                            if st.button(f"🗑️ SUPPRIMER CE CLIENT", key=f"btn_del_{unique_suffix}"):
                                if check_sup:
                                    executer('DELETE FROM clients WHERE "CIN" = %s', (cin_client_actuel,), modifier=True)
                        ...
                        if st.session_state.get(f"mode_edition_{unique_suffix}", False):
                            ...
                            with st.form(key=f"form_reel_edit_{unique_suffix}"):
                                ...
                                try:
                                    e_init_d_cin = datetime.strptime(str(cli.get('Date Délivrance CIN')), "%Y-%m-%d").date()
                                except Exception:
                                    e_init_d_cin = datetime.now().date()
                                try:
                                    e_init_d_per = datetime.strptime(str(cli.get('Date Délivrance Permis')), "%Y-%m-%d").date()
                                except Exception:
                                    e_init_d_per = datetime.now().date()
                                ...
                                if st.form_submit_button("✅ METTRE À JOUR"):
                                    up_sql = 'UPDATE clients SET "Prénom"=%s, "Nom"=%s, "Numéro de téléphone"=%s, "N° Permis"=%s, "Date Délivrance CIN"=%s, "Date Délivrance Permis"=%s'
                                    up_params = [e_prenom, e_nom, e_tel, e_permis, e_d_cin.strftime("%Y-%m-%d"), e_d_per.strftime("%Y-%m-%d")]
                                    if f_cin_remplace:
                                        up_sql += ', "Image CIN"=%s'
                                        up_params.append(encoder_image_base64(f_cin_remplace))
                                    if f_per_remplace:
                                        up_sql += ', "Image Permis"=%s'
                                        up_params.append(encoder_image_base64(f_per_remplace))
                                    up_sql += ' WHERE "CIN"=%s'
                                    up_params.append(cin_client_actuel)
                                    executer(up_sql, tuple(up_params), modifier=True)
        ...
        with st.form("form_new_client_crm"):
            ...
            if st.form_submit_button("⚡ CRÉER LE PROFIL CLIENT"):
                if n_prenom and n_nom and n_cin:
                    executer(
                        'INSERT INTO clients ("Prénom", "Nom", "CIN", "Numéro de téléphone", '
                        '"N° Permis", "Date Délivrance CIN", "Date Délivrance Permis", '
                        '"Image CIN", "Image Permis") '
                        'VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) '
                        'ON CONFLICT ("CIN") DO UPDATE SET '
                        '"Prénom" = EXCLUDED."Prénom", '
                        '"Nom" = EXCLUDED."Nom", '
                        '"Numéro de téléphone" = EXCLUDED."Numéro de téléphone", '
                        '"N° Permis" = EXCLUDED."N° Permis", '
                        '"Date Délivrance CIN" = EXCLUDED."Date Délivrance CIN", '
                        '"Date Délivrance Permis" = EXCLUDED."Date Délivrance Permis", '
                        '"Image CIN" = EXCLUDED."Image CIN", '
                        '"Image Permis" = EXCLUDED."Image Permis"',
                        (n_prenom, n_nom, n_cin, n_tel, n_permis,
                         n_d_cin.strftime("%Y-%m-%d"), n_d_per.strftime("%Y-%m-%d"),
                         encoder_image_base64(f_cin_new), encoder_image_base64(f_per_new)),
                        modifier=True,
                    )
